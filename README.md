...
* HOUSES_DB_ADDR - houses_db_addr - address of the postgres with provision [default: _localhost_] (string)
* HOUSES_DB_PORT - houses_db_port - port of the postgres with provision [default: _5432_] (int)
* HOUSES_DB_NAME - houses_db_name - name of the postgres database with provision [default: _provision_] (string)
* HOUSES_DB_USER - houses_db_user - user name for database [default: _postgres_] (string)
* HOUSES_DB_PASS - houses_db_pass - user password for database [default: _postgres_] (string)
* PROVISION_CITY_NAME - city_name - name of a city to work with
...
* -C,--city_name \<str\> - city_name

## Building Docker image (the other way is to use Docker repository: kanootoko/digitalmodel_provision:2021-10-27)

1. open terminal in cloned repository
2. build image with `docker build --tag kanootoko/digitalmodel_provision:2021-10-27 .`
3. run image with postgres server running on host machine on default port 5432
    1. For windows: `docker run --publish 8080:8080 -e PROVISION_API_PORT=8080 -e HOUSES_DB_ADDR=host.docker.internal -e PROVISION_DB_ADDR=host.docker.internal --name provision_api kanootoko/digitalmodel_provision:2021-10-27`
    2. For Linux: `docker run --publish 8080:8080 -e PROVISION_API_PORT=8080 -e HOUSES_DB_ADDR=$(ip -4 -o addr show docker0 | awk '{print $4}' | cut -d "/" -f 1) -e PROVISION_DB_ADDR=$(ip -4 -o addr show docker0 | awk '{print $4}' | cut -d "/" -f 1) --name provision_api kanootoko/digitalmodel_provision:2021-10-27`  
      Ensure that:
...
* **/api/provision_v3/house_services/{house_id}/**: returns the list of services that are contained by the given living house's normative availability zones.
* **/api/provision_v3/service_houses/{service_id}/**: returns the list of houses that contain the given service in their normaive availability zone.
